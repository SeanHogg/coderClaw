...
SMOKE_PREVIOUS_VERSION="${CODERCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${CODERCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (CODERCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${CODERCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${CODERCLAW_INSTALL_LATEST_OUT:-}"
fi
