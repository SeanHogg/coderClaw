...
describe("dedupeSkillCommandsForDiscord", () => {
  ...
  it("treats skillName case-insensitively", () => {
    const input = [
      { name: "WebSearch", skillName: "WebSearch", description: "Web search skill" },
      { name: "websearch_2", skillName: "websearch", description: "Web search skill" },
    ];
    const output = __testing.dedupeSkillCommandsForDiscord(input);
    expect(output).toHaveLength(1);
    expect(output[0]?.name).toBe("WebSearch");
  });
});
