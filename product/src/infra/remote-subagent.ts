/**
 * Low-level Builderforce remote-claw HTTP helpers.
 *
 * Exports:
 *   - `fetchFleetEntries`   — `GET /api/claws/fleet` (claw-authenticated).
 *   - `dispatchToRemoteClaw` — `POST /api/claws/:targetId/forward` with HMAC payload.
 *   - `dispatchResultToRemoteClaw` — callback path for task results.
 *
...
/** Fetch the raw claw fleet entries for the current tenant. */
