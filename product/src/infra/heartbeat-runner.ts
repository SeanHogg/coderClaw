...
import { DEFAULT_HEARTBEAT_FILENAME, resolveWorkspaceFilePath } from "../agents/workspace.js";
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: CoderClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: CoderClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: CoderClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: CoderClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: CoderClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: CoderClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export async function runHeartbeatOnce(opts: {
  ...
  const heartbeatFilePath = resolveWorkspaceFilePath(workspaceDir, DEFAULT_HEARTBEAT_FILENAME);
  try {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: CoderClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
