...
import { resolveCoderClawDocsPath } from "../../docs-path.js";
...
import { createCoderClawCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveCoderClawAgentDir();

    // Check if the model supports native image input
    const modelHasVision = params.model.input?.includes("image") ?? false;
    const toolsRaw = params.disableTools
      ? []
      : createCoderClawCodingTools({
          exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveCoderClawDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // Local brain: when enabled, always use amygdala → hippocampus → cortex.
      // When disabled, go directly to cortex (SDK stream).
      ...
      const localBrainEnabled =
        localBrainEnv !== undefined
          ...
          : params.config?.localBrain?.enabled === true;

      if (localBrainEnabled) {
        // Amygdala (SmolLM2) routes; hippocampus (Phi-4) plans; cortex (user's LLM) executes on DELEGATE.
        log.info(
        ...
        const amygdalaModelId = localBrainModels?.amygdala?.modelId;
        const amygdalaDtype = localBrainModels?.amygdala?.dtype;
      ...
      try {
        ...
        // double-compaction. See: https://github.com/SeanHogg/coderClaw/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt") {
          try {
            sessionManager.appendCustomEntry("coderclaw:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/SeanHogg/coderClaw/issues/8643
