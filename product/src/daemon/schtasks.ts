...
export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.CODERCLAW_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.CODERCLAW_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
export function buildTaskLauncher(cmdScriptPath: string): string {
  ...
  return (
    [
      ...
      "}",
      "",
      "# Run the gateway (Start-Process Hidden avoids cmd.exe window flash)",
      "$proc = Start-Process -FilePath 'cmd.exe' -ArgumentList '/c', $cmdScript -WindowStyle Hidden -Wait -PassThru",
