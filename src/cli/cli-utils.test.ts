...
describe("shouldSkipRespawnForArgv", () => {
  it("skips respawn for help/version calls", () => {
    expect(shouldSkipRespawnForArgv(["node", "coderclaw", "--help"])).toBe(true);
    expect(shouldSkipRespawnForArgv(["node", "coderclaw", "-V"])).toBe(true);
  });

  it("keeps respawn path for normal commands", () => {
    expect(shouldSkipRespawnForArgv(["node", "coderclaw", "status"])).toBe(false);
  });
});
