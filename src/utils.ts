...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".coderclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.CODERCLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$CODERCLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via CODERCLAW_STATE_DIR.
