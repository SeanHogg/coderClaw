...
describe("QmdMemoryManager", () => {
  ...
  let stateDir: string;
  let cfg: CoderClawConfig;
  const agentId = "main";

  async function createManager(params?: { mode?: "full" | "status"; cfg?: CoderClawConfig }) {
  ...
  it("rebuilds managed collections once when qmd update fails with null-byte ENOTDIR", async () => {
    ...
    expect(removeCalls).toEqual(["memory-root-main", "memory-alt-main", "memory-dir-main", "coderclaw-memory-dir-main"]);
    expect(addCalls).toEqual(["memory-root-main", "memory-alt-main", "memory-dir-main", "coderclaw-memory-dir-main"]);
