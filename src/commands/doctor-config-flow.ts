...
function stripUnknownConfigKeys(config: CoderClawConfig): {
  config: CoderClawConfig;
  removed: string[];
} {
  const parsed = CoderClawSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: CoderClawConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: CoderClawConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: CoderClawConfig): Promise<{
  config: CoderClawConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: CoderClawConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: CoderClawConfig): {
  config: CoderClawConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: CoderClawConfig): {
  config: CoderClawConfig;
  changes: string[];
} {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".coderclaw");
  const targetPath = path.join(targetDir, "coderclaw.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".coderclaw", "coderclaw.json"),
    path.join(home, ".coderclaw", "coderclaw.json"),
    path.join(home, ".coderclaw", "coderclaw.json"),
  ];
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: CoderClawConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("coderclaw doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("coderclaw doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("coderclaw doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("coderclaw doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("coderclaw doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "coderclaw doctor --fix" to remove these keys.');
    }
  }
