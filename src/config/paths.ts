...
const LEGACY_CONFIG_FILENAMES = ["openclaw.json", "clawdbot.json", "moldbot.json", "moltbot.json"] as const;
...
export function resolveStateDir(
  ...
  const override = env.CODERCLAW_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
...
export function resolveCanonicalConfigPath(
  ...
  const override = env.CODERCLAW_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.CODERCLAW_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const coderclawStateDir = env.CODERCLAW_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
...
export function resolveGatewayPort(
  ...
  const envRaw = env.CODERCLAW_GATEWAY_PORT?.trim() || env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
