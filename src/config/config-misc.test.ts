...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = CoderClawSchema.safeParse({
      $schema: "https://coderclaw.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://coderclaw.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = CoderClawSchema.safeParse({ $schema: 123 });
