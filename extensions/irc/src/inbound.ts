...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as CoderClawConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as CoderClawConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as CoderClawConfig);
