...
This document describes how CoderClaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

CoderClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, CoderClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
├── pi-tools.ts                    # createCoderClawCodingTools()
...
├── coderclaw-tools.ts              # CoderClaw-specific tools
...
2. **Custom Replacements**: CoderClaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **CoderClaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures CoderClaw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, CoderClaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
CoderClaw wraps this with `guardSessionManager()` for tool result safety.
...
CoderClaw maintains an auth profile store with multiple API keys per provider:
...
CoderClaw loads custom pi extensions for specialized behavior:
...
CoderClaw also has a local TUI mode that uses pi-tui components directly:
...
- `src/agents/pi-tools.create-coderclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-coderclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-coderclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-coderclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
