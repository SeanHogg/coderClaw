...
- The regexes are case-insensitive; they cover a display-name ping like `@coderclaw` and the raw number with or without `+`/spaces.
...
- Manual smoke:
  - Send an `@coderclaw` ping in the group and confirm a reply that references the sender name.
...
- Session store entries will appear as `agent:<agentId>:whatsapp:group:<jid>` in the session store (`~/.coderclaw/agents/<agentId>/sessions/sessions.json` by default); a missing entry just means the group hasn’t triggered a run yet.
