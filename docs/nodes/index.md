...
- The token is `gateway.auth.token` from the gateway config (`~/.coderclaw/openclaw.json` on the gateway host).
- `coderclaw node run` reads `OPENCLAW_GATEWAY_TOKEN` for auth.
