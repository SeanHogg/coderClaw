...
OpenClaw uses a dedicated workspace directory for the agent. Default: `~/.coderclaw/workspace` (configurable via `agents.defaults.workspace`).
...
- For browser-driven verification, use `coderclaw browser` (tabs/status/screenshot) with the OpenClaw-managed Chrome profile.
