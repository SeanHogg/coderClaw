...
- Re-open anytime: `coderclaw dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
...
- Ensure the gateway is reachable (local: `coderclaw status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
- Retrieve the token from the gateway host: `coderclaw config get gateway.auth.token` (or generate one: `coderclaw doctor --generate-gateway-token`).
