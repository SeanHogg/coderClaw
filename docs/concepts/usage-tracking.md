...
- CLI: `coderclaw status --usage` prints a full per-provider breakdown.
- CLI: `coderclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
