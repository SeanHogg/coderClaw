...
- Secrets live in `~/.coderclaw/agents/<agentId>/agent/auth-profiles.json` (legacy: `~/.coderclaw/agent/auth-profiles.json`).
- Config `auth.profiles` / `auth.order` are **metadata + routing only** (no secrets).
- Legacy import-only OAuth file: `~/.coderclaw/credentials/oauth.json` (imported into `auth-profiles.json` on first use).
