...
- `commands.config` (default `false`) enables `/config` (reads/writes `coderclaw.json`).
...
- For full provider usage breakdown, use `coderclaw status --usage`.
...
- Overrides apply immediately to new config reads, but do **not** write to `coderclaw.json`.
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/coderclaw`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
