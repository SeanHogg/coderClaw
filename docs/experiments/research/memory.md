---
summary: "Research notes: offline memory system for CoderClaw workspaces (Markdown source-of-truth + derived index)"
read_when:
...
Target: CoderClaw-style workspace (`agents.defaults.workspace`, default `~/.coderclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.coderclaw/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want CoderClaw to see every session.
...
- You want the agent itself to call the tools:
  - `coderclaw memory recall "…" --k 25 --since 30d`
  - `coderclaw memory reflect --since 7d`
...
Pragmatic take for `~/.coderclaw/workspace`:
