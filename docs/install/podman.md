...
This also creates a minimal `~openclaw/.openclaw/coderclaw.json` (sets `gateway.mode="local"`) so the gateway can start without running the wizard.
...
- **Home:** e.g. `/home/openclaw` — holds `~/.coderclaw` (config, workspace) and the launch script `run-coderclaw-podman.sh`.
- **Rootless Podman:** The user must have a **subuid** and **subgid** range. Many distros assign these automatically when the user is created. If setup prints a warning, add lines to `/etc/subuid` and `/etc/subgid`:
  ...
  ```bash
  sudo -u coderclaw /home/openclaw/run-coderclaw-podman.sh
  sudo -u coderclaw /home/openclaw/run-coderclaw-podman.sh setup
  ```

- **Config:** Only `openclaw` and root can access `/home/openclaw/.openclaw`. To edit config: use the Control UI once the gateway is running, or `sudo -u coderclaw $EDITOR /home/openclaw/.openclaw/coderclaw.json`.
...
- **Paths:** Host config and workspace default to `~openclaw/.openclaw` and `~openclaw/.openclaw/workspace`. Override the host paths used by the launch script with `CODERCLAW_CONFIG_DIR` and `CODERCLAW_WORKSPACE_DIR`.

## Useful commands

- **Logs:** With quadlet: `sudo journalctl --machine coderclaw@ --user -u coderclaw.service -f`. With script: `sudo -u coderclaw podman logs -f openclaw`
- **Stop:** With quadlet: `sudo systemctl --machine coderclaw@ --user stop coderclaw.service`. With script: `sudo -u coderclaw podman stop openclaw`
- **Start again:** With quadlet: `sudo systemctl --machine coderclaw@ --user start coderclaw.service`. With script: re-run the launch script or `podman start openclaw`
- **Remove container:** `sudo -u coderclaw podman rm -f openclaw` — config and workspace on the host are kept
...
- **Gateway start blocked (missing `gateway.mode=local`):** Ensure `~openclaw/.openclaw/coderclaw.json` exists and sets `gateway.mode="local"`. `setup-podman.sh` creates this file if missing.
...
- **Script not found when running as openclaw:** Ensure `setup-podman.sh` was run so that `run-coderclaw-podman.sh` is copied to openclaw’s home (e.g. `/home/openclaw/run-coderclaw-podman.sh`).
