...
See [`ClawDock` Helper README](https://github.com/SeanHogg/coderClaw/blob/main/scripts/shell-helpers/README.md) for details.
...
- If you edit `CODERCLAW_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
- If you change `CODERCLAW_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
- If you change `CODERCLAW_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `CODERCLAW_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `CODERCLAW_EXTRA_MOUNTS`.
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/SeanHogg/coderClaw/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
