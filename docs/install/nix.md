...
When `CODERCLAW_NIX_MODE=1` is set (automatic with nix-openclaw):
...
OpenClaw reads JSON5 config from `CODERCLAW_CONFIG_PATH` and stores mutable data in `CODERCLAW_STATE_DIR`.
