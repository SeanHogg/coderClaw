...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.coderclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.coderclaw/node.json` (node id + pairing token).
...
Path: `~/.coderclaw/exec-approvals.json`
...
- Unix socket at `~/.coderclaw/exec-approvals.sock` (0600).
