...
```bash
coderclaw plugins enable minimax-portal-auth  # 如果已加载则跳过
coderclaw gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/SeanHogg/coderClaw/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `coderclaw models list` 和 `coderclaw models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `coderclaw configure` 并选择 **MiniMax M2.1**，或
