...
如果页面加载失败，请先启动 Gateway 网关：`coderclaw gateway`。
...
一旦批准，设备会被记住，除非你使用 `coderclaw devices revoke --device <id> --role <role>` 撤销它，否则不需要重新批准。参见 [Devices CLI](/cli/devices) 了解 token 轮换和撤销。
...
- 配置：查看/编辑 `~/.openclaw/coderclaw.json`（`config.get`、`config.set`）
...
```bash
CODERCLAW_CONTROL_UI_BASE_PATH=/openclaw/ pnpm ui:build
```
