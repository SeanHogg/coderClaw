...
7. 直接聊天：默认通过 `channels.discord.dm.policy` 进行安全保护（默认：`"pairing"`）。未知发送者会收到配对码（1 小时后过期）；通过 `coderclaw pairing approve discord <code>` 批准。
...
- 首先：运行 `coderclaw doctor` 和 `coderclaw channels status --probe`（可操作的警告 + 快速审计）。
...
- 如果机器人卡住或受到速率限制，在确认没有其他进程拥有 Discord 会话后重启 Gateway 网关（`coderclaw gateway --force`）。
