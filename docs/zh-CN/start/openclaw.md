...
3. 在 `~/.openclaw/coderclaw.json` 中放置最小配置：
...
```bash
coderclaw status          # 本地状态（凭证、会话、排队事件）
coderclaw status --all    # 完整诊断（只读，可粘贴）
coderclaw status --deep   # 添加 Gateway 网关健康探测（Telegram + Discord）
coderclaw health --json   # Gateway 网关健康快照（WS）
```
