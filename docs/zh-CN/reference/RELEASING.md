...
- [ ] 更新 CLI/版本字符串：[`src/cli/program.ts`](https://github.com/SeanHogg/coderClaw/blob/main/src/cli/program.ts) 和 [`src/provider-web.ts`](https://github.com/SeanHogg/coderClaw/blob/main/src/provider-web.ts) 中的 Baileys user agent。
- [ ] 确认包元数据（name、description、repository、keywords、license）以及 `bin` 映射指向 [`coderclaw.mjs`](https://github.com/SeanHogg/coderClaw/blob/main/coderclaw.mjs) 作为 `openclaw`。
- [ ] 如果依赖项有变化，运行 `pnpm install` 确保 `pnpm-lock.yaml` 是最新的。

2. **构建和产物**

- [ ] 如果 A2UI 输入有变化，运行 `pnpm canvas:a2ui:bundle` 并提交更新后的 [`src/canvas-host/a2ui/a2ui.bundle.js`](https://github.com/SeanHogg/coderClaw/blob/main/src/canvas-host/a2ui/a2ui.bundle.js)。
...
- [ ] `CODERCLAW_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`（Docker 安装冒烟测试，快速路径；发布前必需）
  - 如果已知上一个 npm 发布版本有问题，为预安装步骤设置 `CODERCLAW_INSTALL_SMOKE_PREVIOUS=<last-good-version>` 或 `CODERCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1`。
...
- [ ] 生成 Sparkle appcast（通过 [`scripts/make_appcast.sh`](https://github.com/SeanHogg/coderClaw/blob/main/scripts/make_appcast.sh) 生成 HTML 注释）并更新 `appcast.xml`。
...
- [ ] 验证注册表：`npm view coderclaw version`、`npm view coderclaw dist-tags` 和 `npx -y openclaw@X.Y.Z --version`（或 `--help`）。
