...
3. **全局 `.env`** 位于 `~/.openclaw/.env`（即 `$CODERCLAW_STATE_DIR/.env`；不覆盖）。
4. **配置 `env` 块** 位于 `~/.openclaw/coderclaw.json`（仅在缺失时应用）。
5. **可选的登录 shell 导入**（`env.shellEnv.enabled` 或 `CODERCLAW_LOAD_SHELL_ENV=1`），仅对缺失的预期键名应用。
