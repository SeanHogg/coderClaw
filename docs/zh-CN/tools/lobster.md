...
在运行 OpenClaw Gateway 网关的**同一主机**上安装 Lobster CLI（参见 [Lobster 仓库](https://github.com/SeanHogg/lobster)），并确保 `lobster` 在 `PATH` 中。
...
```
用户："检查我的邮件并起草回复"
→ coderclaw 调用 gmail.list
→ LLM 总结
→ 用户："给 #2 和 #5 起草回复"
→ LLM 起草
→ 用户："发送 #2"
→ coderclaw 调用 gmail.send
（每天重复，不记得已分类的内容）
```
