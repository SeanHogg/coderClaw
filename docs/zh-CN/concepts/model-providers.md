...
- CLI 辅助工具：`coderclaw onboard`、`coderclaw models list`、`coderclaw models set <provider/model>`。
...
- CLI：`coderclaw onboard --auth-choice token`（粘贴 setup-token）或 `coderclaw models auth paste-token --provider anthropic`
...
- CLI：`coderclaw onboard --auth-choice openai-codex` 或 `coderclaw models auth login --provider openai-codex`
...
- Antigravity OAuth 作为捆绑插件提供（`google-antigravity-auth`，默认禁用）。
  - 启用：`coderclaw plugins enable google-antigravity-auth`
  - 登录：`coderclaw models auth login --provider google-antigravity --set-default`
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  - 启用：`coderclaw plugins enable google-gemini-cli-auth`
  - 登录：`coderclaw models auth login --provider google-gemini-cli --set-default`
  - 注意：你**不需要**将客户端 ID 或密钥粘贴到 `coderclaw.json` 中。CLI 登录流程将令牌存储在 Gateway 网关主机的认证配置文件中。
...
coderclaw models set opencode/claude-opus-4-5
